...
function materialInfoController($scope, Restangular, NgTableParams, dialogs, toaster, $http, $rootScope, $filter) {
	...
	function materialInfoAll() {
		...
		Restangular.all('/epeins-factory/MaterialInfo/query').post(params).then(function(res) {
	...
	function search(){
		...
		//if ($scope.data.title || $scope.data.startDate || $scope.data.endDate) {
			...
			var params = { 
				id: $scope.data.id || '',
				materialName: $scope.data.materialName || '',
				materialDisp: $scope.data.materialDisp || '',
				materialColor: $scope.data.materialColor || '',
...
app.controller('addMaterialInfoController',function($scope, $modalInstance, Restangular, data, toaster, $rootScope){
	...
	function submit(){
		...
		// 调接口，储存
		Restangular.all('/epeins-factory/MaterialInfo/addOrUpdate').post(item).then(function(res) {
...
app.controller('modifyMaterialInfoController',function($scope, $modalInstance, data, Restangular, $rootScope, toaster){
	...
	function submit(){
		var item = {
			id: $scope.details.id,
			materialName: $scope.details.materialName,
			materialDisp: $scope.details.materialDisp,
			materialColor: $scope.details.materialColor,
...
app.controller('deteleMaterialInfoController',function($scope, $modalInstance, data, Restangular, $rootScope,toaster){
	...
	function submit(id) {
		...
		Restangular.one('/epeins-factory/MaterialInfo/delete').customGET('',params).then(function(res) {
			if (res.resultCode == 200) {
				//请求删除数据
