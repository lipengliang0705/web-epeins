app.controller('checkStandardController', checkStandardController);
checkStandardController.$inject = ['$scope', 'Restangular' ,'ngTableParams', 'dialogs', 'toaster', '$http', '$rootScope', '$filter'];
function checkStandardController($scope, Restangular, NgTableParams, dialogs, toaster, $http, $rootScope, $filter) {
	$scope.resoures = {
		list: [],//信息列表
	};
	...
	//临时数据
	$scope.prodTestData = [
		{platNo:'1', stdCode:"CHEC-TM-20-B/A", prodName:"YFL-1-T/B", clientId:"南通神华", materialId:"PA30", memo:"任务单信息", changeFlg:"true"},
	];
	//临时赋予数据
	...
	function add(item) {
		var dlg = dialogs.create('views/tpl/basic-data/check-standard/add.html','addCheckStandardController',{data:item},{size:'lg'});
    };
	
    function modify(item) {
		var dlg = dialogs.create('views/tpl/basic-data/check-standard/modify.html','modifyCheckStandardController',{data:item},{size:'lg'});	
    };

    function deleted(item) {
		var dlg = dialogs.create('views/tpl/basic-data/check-standard/delete.html','deteleCheckStandardController',{data:item},{size:'sm'});	
	};
	$scope.data = {
		search:'',
	};
...
app.controller('addCheckStandardController',function($scope, $modalInstance, Restangular, data, toaster, $rootScope){
	$scope.title = "新增产品检验标准数据";
	//临时测试数据
	// $scope.prodUuid = [
...
app.controller('modifyCheckStandardController',function($scope, $modalInstance, data, Restangular, $rootScope, toaster){
	$scope.title = "产品检验标准变更";
	//选中的行赋给details数组
...
app.controller('deteleCheckStandardController',function($scope, $modalInstance, data, Restangular, $rootScope,toaster){
	console.log(data);
	$scope.title = "删除产品检验标准数据";
	$scope.data = {
		id: data.data,
	}
	// 方法
